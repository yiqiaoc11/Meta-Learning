...
logger = logging.getLogger('omniglot_AutoML')
...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        dummy_labels = torch.empty_like(input_labels)
        dummy_labels.copy_(input_labels)
        dummy_labels[:,-1,:,:] = torch.zeros(self.num_classes, self.num_classes)
        input_images = torch.cat((input_images, dummy_labels), -1)

        input_images = torch.reshape(input_images, (batch_size, -1, self.num_classes + 784))

        # h0 = torch.randn(1, batch_size, hidden)
        # c0 = torch.randn(1, batch_size, hidden)
        # h1 = torch.randn(1, batch_size, self.num_classes)
        # c1 = torch.randn(1, batch_size, self.num_classes)

        o1, _ = self.layer1(input_images) #, (h0, c0))
        o2, _ = self.layer2(o1) #, (h1, c1))
        return torch.reshape(o2, (batch_size, self.samples_per_class, self.num_classes, self.num_classes))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--meta_batch_size", type=int, default=128, metavar='N')
